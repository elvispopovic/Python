...
class PrimanjePoruka(spade.Behaviour.Behaviour):
    ...
    def dopuniListuPodataka(self,content):
        ...
        if rowsplt[0]=="kut":
            stanjeAgenta.kut = float(rowsplt[1])*3.1415927/180.0
        else:
...
class Azuriranje(spade.Behaviour.OneShotBehaviour):
    ...
    def _process(self):
        ...
        #self.myAgent.protokol.izracunaj(self.myAgent.koordinate,self.myAgent.lista_koordinata_susjeda,0.05, 0.5)
        self.myAgent.protokol.izracunaj(self.myAgent.koordinate,self.myAgent.lista_koordinata_susjeda)
        self.myAgent.protokol.normiraj(self.myAgent.koordinate,0.8)
        self.myAgent.protokol.integriraj(self.myAgent.koordinate)
        sadrzajIspisa = ""
        #for a in self.myAgent.lista_koordinata_susjeda:
        for i in range(0,len(lista_susjeda)):
            a = lista_susjeda[i]
            if a != None:
                sadrzajIspisa += "{0:s}: Stanje susjednog agenta {1:d}: radijus: {2:.2f}, kut: {3:.4f}, polozaj x: {4:.2f}, y: {5:.2f},\
brzina x: {6:.2f}, y: {7:.2f}{8:s}".format(ime_agenta,a.brojAgenta,a.radijus,a.kut,a.polozaj[0],a.polozaj[1],a.brzina[0],a.brzina[1],os.linesep)
        #    else:
