...
class Igrac(spade.Agent.Agent):
        ...
        def _process(self):
            ...
            while True:
                ...
                if self.msg.getOntology()!="Pogodi":
                    ...
                    break;
            if(self.myAgent.min<self.myAgent.max):
                self.myAgent.zamisljeniBroj = randint(self.myAgent.min,self.myAgent.max)
            else:
                self.myAgent.zamisljeniBroj = self.myAgent.min
            self.myAgent.Ispis("%s: Pokušavam sa brojem %d" %(self.myAgent.ime,self.myAgent.zamisljeniBroj))
            self.myAgent.SlanjePoruke("Pogadjanje",self.myAgent.suprotni,str(self.myAgent.zamisljeniBroj))
            #primanje odgovora
            while True:
                self.msg = self.myAgent.SkidanjePoruke()
                if self.msg.getOntology()!="Rezultat":
                    sleep(1)
                else:
                    break

            if self.msg.getContent()=="tocno":
                self.myAgent.naRedu = True
                self._exitcode = self.myAgent.POGODIO_SAM
                return
            elif self.msg.getContent()=="premalo":
                if(self.myAgent.verbose):
                    self.myAgent.Ispis("%s: povecavam donju granicu na %d." %(self.myAgent.ime,self.myAgent.zamisljeniBroj+1));
                self.myAgent.min=self.myAgent.zamisljeniBroj+1
            else:
                if(self.myAgent.verbose):
                    self.myAgent.Ispis("%s: smanjujem gornju granicu na %d." %(self.myAgent.ime,self.myAgent.zamisljeniBroj-1));
                self.myAgent.max=self.myAgent.zamisljeniBroj-1
            if(self.myAgent.verbose):
                self.myAgent.Ispis("%s: Raspon brojeva je od %d do %d." %(self.myAgent.ime,self.myAgent.min,self.myAgent.max))
    ...
    class ProvjeraPogadjanja(spade.Behaviour.OneShotBehaviour):
        def _process(self):
            ...
            if int(self.msg.getContent())==self.myAgent.zamisljeniBroj:
                rezultat = "tocno"
                self.myAgent.Ispis("%s: >>> Pogodio si! <<<"%self.myAgent.ime)
                self.myAgent.naRedu = False
                self.myAgent.Ispis("%s: Mijenjamo se."%(self.myAgent.ime))
                self.myAgent.SlanjePoruke("Rezultat",self.myAgent.suprotni,rezultat)
                self.myAgent.min = 1
                self.myAgent.max = 10
                self._exitcode = self.myAgent.POGODIO_JE
                return
            elif int(self.msg.getContent())<self.myAgent.zamisljeniBroj:
                rezultat = "premalo"
                self.myAgent.Ispis("%s: Premalo!"%self.myAgent.ime)
            else:
                rezultat = "previse"
                self.myAgent.Ispis("%s: Previše!"%self.myAgent.ime)
            self.myAgent.SlanjePoruke("Rezultat",self.myAgent.suprotni,rezultat)
    ...
    def SlanjePoruke(self, ontologija, primatelj, sadrzaj):
        ...
        self.send( self.msg )
        if(self.verbose):
            self.Ispis("%s: poslana je poruka ontologije \"%s\" i sadrzaja \"%s\""%(self.ime,self.msg.getOntology(),self.msg.getContent()),True)
